...
for i in $(ls rainbow_frames/1_intro); do
  ...
  # rm -rf _*
  osascript -e 'tell application "Finder" to delete (make new folder at (front window))' > ./rainbow_frames/unused/err.txt;
  # osascript -e 'tell application "Finder" to delete (make new folder at (front window) with properties {name:"_"})' > ./rainbow_frames/unused/err.txt;
done
